...
if __name__ == '__main__':
    ...
    X = [[0.23,0.21], [0.17,0.22], [0.22,0.22], [0.16,0.20],
         [0.22,0.17], [0.19,0.18], [0.21,0.20]]
    y = [-1, 1, -1, 1, 1, -1, 1]
    X = np.array(X)
    y = np.array(y)
    testPoint = np.array([0.2, 0.2])

    plt.figure(figsize=(15, 8))
    plt.scatter(X[y==-1, 0], X[y==-1, 1], marker='o', c='red', label='Classe -1', s=60);
    plt.scatter(X[y==1, 0], X[y==1, 1], marker='o', c='blue', label='Classe 1', s=60);
    plt.scatter(testPoint[0], testPoint[1], marker='o', c='black', label='Ponto P (sem classe)',s=60);
    
    r = distance.euclidean(testPoint, X[0])
    c = plt.Circle((testPoint[0], testPoint[1]), r, color='black', fill=False)
    plt.gca().add_patch(c)

    r = distance.euclidean(testPoint, X[3])
    c = plt.Circle((testPoint[0], testPoint[1]), r, color='black', fill=False)
    plt.gca().add_patch(c)

    plt.annotate('K = 4', (0.2, 0.232), fontweight='bold', fontsize=12)
    plt.annotate('K = 7', (0.2, 0.241), fontweight='bold', fontsize=12)

    plt.axis("equal")
    plt.legend(prop={'size': 16, 'weight':'bold'})
    plt.savefig('simpleKNN.png')
    plt.show()
