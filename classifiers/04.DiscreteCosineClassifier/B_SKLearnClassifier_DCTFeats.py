...
augmPatLvDiv_TRAIN = Path('data/AugmPatLvDiv_TRAIN-DCTData_1024-Features_20000-images.csv')
augmPatLvDiv_VALIDATION = Path('data/AugmPatLvDiv_VALIDATION-DCTData_1024-Features_5000-images.csv')
augmRndDiv_TRAIN = Path('data/AugmRndDiv_TRAIN-DCTData_1024-Features_20000-images.csv')
augmRndDiv_VALIDATION = Path('data/AugmRndDiv_VALIDATION-DCTData_1024-Features_5000-images.csv')
patLvDiv_TEST = Path('data/PatLvDiv_TEST-DCTData_1024-Features_550-images.csv')
patLvDiv_TRAIN = Path('data/PatLvDiv_TRAIN-DCTData_1024-Features_7224-images.csv')
patLvDiv_VALIDATION = Path('data/PatLvDiv_VALIDATION-DCTData_1024-Features_2887-images.csv')
rndDiv_TEST = Path('data/rndDiv_TEST-DCTData_1024-Features_1066-images.csv')
rndDiv_TRAIN = Path('data/rndDiv_TRAIN-DCTData_1024-Features_6398-images.csv')
rndDiv_VALIDATION = Path('data/rndDiv_VALIDATION-DCTData_1024-Features_3197-images.csv')
...
if __name__ == '__main__':
    ...
    pcaNofFeat = [50, 100, 150, 200, 300, 400, 600, 800, 1024]
    ...
    def LSVMaugmPatLvDiv():
        ...
        df.to_csv(f'results/L-SVM_ValidPerformance_DCTData_AugmPatLvDivImages.csv')
    ###############################################################################
    def LSVMaugmRndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'L-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/L-SVM_ValidPerformance_DCTData_AugmRndDivImages.csv')
    ###############################################################################
    def LSVMpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'L-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/L-SVM_ValidPerformance_DCTData_PatLvDivImages.csv')
    ###############################################################################
    def LSVMrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'L-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/L-SVM_ValidPerformance_DCTData_RndDivImages.csv')
    ###############################################################################
    ...
    def QSVMaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Q-SVM_ValidPerformance_DCTData_AugmPatLvDivImages.csv')
    ###############################################################################
    def QSVMaugmRndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'Q-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Q-SVM_ValidPerformance_DCTData_AugmRndDivImages.csv')
    ###############################################################################
    def QSVMpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'Q-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Q-SVM_ValidPerformance_DCTData_PatLvDivImages.csv')
    ###############################################################################
    def QSVMrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'Q-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Q-SVM_ValidPerformance_DCTData_RndDivImages.csv')
    ###############################################################################
    ...
    def PolySVMaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/P-SVM_ValidPerformance_DCTData_AugmPatLvDivImages.csv')
    ###############################################################################
    def PolySVMaugmRndDiv():
        ...
        for feat in pcaNofFeat:
            ...
            collectedData[f'P-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/P-SVM_ValidPerformance_DCTData_AugmRndDivImages.csv')
    ###############################################################################
    def PolySVMpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'P-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/P-SVM_ValidPerformance_DCTData_PatLvDivImages.csv')
    ###############################################################################
    def PolySVMrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'P-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/P-SVM_ValidPerformance_DCTData_RndDivImages.csv')
    ###############################################################################
    ...
    def RBFSVMaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/R-SVM_ValidPerformance_DCTData_AugmPatLvDivImages.csv')
    ###############################################################################
    def RBFSVMaugmRndDiv():
        ...
        for feat in pcaNofFeat:
            ...
            collectedData[f'R-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/R-SVM_ValidPerformance_DCTData_AugmRndDivImages.csv')
    ###############################################################################
    def RBFSVMpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'R-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/R-SVM_ValidPerformance_DCTData_PatLvDivImages.csv')
    ###############################################################################
    def RBFSVMrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'R-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/R-SVM_ValidPerformance_DCTData_RndDivImages.csv')
    ###############################################################################
    ...
    def SigmoidSVMaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Sig-SVM_ValidPerformance_DCTData_AugmPatLvDivImages.csv')
    ###############################################################################
    def SigmoidSVMaugmRndDiv():
        ...
        for feat in pcaNofFeat:
            ...
            collectedData[f'S-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Sig-SVM_ValidPerformance_DCTData_AugmRndDivImages.csv')
    ###############################################################################
    def SigmoidSVMpatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            collectedData[f'S-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Sig-SVM_ValidPerformance_DCTData_PatLvDivImages.csv')
    ###############################################################################
    def SigmoidSVMrndDiv():
        ...
        for feat in pcaNofFeat:
            ...
            collectedData[f'S-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Sig-SVM_ValidPerformance_DCTData_RndDivImages.csv')
    ###############################################################################
    ...
    def KNNaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/KNN_ValidPerformance_DCTData_AugmPatLvDivImages.csv')
    ###############################################################################
    def KNNaugmRndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for k in range(5,21):
                ...
                collectedData[f'{k}-KNN_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/KNN_ValidPerformance_DCTData_AugmRndDivImages.csv')
    ###############################################################################
    def KNNpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for k in range(5,21):
                ...
                collectedData[f'{k}-KNN_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/KNN_ValidPerformance_DCTData_PatLvDivImages.csv')
    ###############################################################################
    def KNNrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for k in range(5,21):
                ...
                collectedData[f'{k}-KNN_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/KNN_ValidPerformance_DCTData_rndDivImages.csv')
    ###############################################################################
    ...
    def RFaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/RF_ValidPerformance_DCTData_AugmPatLvDivImages.csv')
    ###############################################################################
    def RFaugmRndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for nEstm in range(50,170,20):
                ...
                collectedData[f'{nEstm}-RF_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/RF_ValidPerformance_DCTData_AugmRndDivImages.csv')
    ###############################################################################
    def RFpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for nEstm in range(50,170,20):
                ...
                collectedData[f'{nEstm}-RF_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/RF_ValidPerformance_DCTData_PatLvDivImages.csv')
    ###############################################################################
    def RFrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for nEstm in range(50,170,20):
                ...
                collectedData[f'{nEstm}-RF_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/RF_ValidPerformance_DCTData_RndDivImages.csv')
    ###############################################################################
