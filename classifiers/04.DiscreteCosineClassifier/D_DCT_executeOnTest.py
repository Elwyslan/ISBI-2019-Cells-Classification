...
from B_SKLearnClassifier_DCTFeats import balanceDataset, computeStats, \
                                         Xy_Split, validateCLF

from B_SKLearnClassifier_DCTFeats import L_SVM_CLF, Q_SVM_CLF, Poly_SVM_CLF, RBF_SVM_CLF, Sigm_SVM_CLF,\
                                         KNN_CLF, RF_CLF
...
def testLSVM():
    ...
    lsvm_df = testSVMClassifier(testID='LSVM-augmPatLvDiv',
                                ...
                                funcParams={'params':{'C':1.0, 'tol':0.001, 'max_iter':-1}, 'PCA_Feats':400},
                                results_df=lsvm_df)
    ...
    lsvm_df = testSVMClassifier(testID='LSVM-patLvDiv',
                                ...
                                funcPointer=L_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'tol':0.001, 'max_iter':-1}, 'PCA_Feats':50},
                                results_df=lsvm_df)
    ...
    lsvm_df.to_csv(Path('results/L-SVM_TestPerformance_DCTData.csv'))
...
def testQSVM():
    ...
    qsvm_df = testSVMClassifier(testID='QSVM-augmPatLvDiv',
                                ...
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':1024},
                                results_df=qsvm_df)
    ...
    qsvm_df.to_csv(Path('results/Q-SVM_TestPerformance_DCTData.csv'))
...
def testPSVM():
    ...
    psvm_df = testSVMClassifier(testID='PSVM-augmPatLvDiv',
                                ...
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':1e6}, 'PCA_Feats':600},
                                results_df=psvm_df)
    psvm_df = testSVMClassifier(testID='PSVM-augmRndDiv',
                                train_df=augmRndDiv_TRAIN,
                                funcPointer=Poly_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':1e6}, 'PCA_Feats':1024},
                                results_df=psvm_df)
    ...
    psvm_df.to_csv(Path('results/P-SVM_TestPerformance_DCTData.csv'))
...
def testRSVM():
    ...
    rsvm_df = testSVMClassifier(testID='RSVM-augmPatLvDiv',
                                ...
                                funcParams={'params':{'C':1.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':300},
                                results_df=rsvm_df)
    ...
    rsvm_df = testSVMClassifier(testID='RSVM-rndDiv',
                                train_df=rndDiv_TRAIN,
                                funcPointer=RBF_SVM_CLF,
                                funcParams={'params':{'C':1.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':600},
                                results_df=rsvm_df)
    cols = rsvm_df.columns.tolist()
    cols.remove('testID')
    rsvm_df = rsvm_df[['testID'] + cols]
    rsvm_df.to_csv(Path('results/R-SVM_TestPerformance_DCTData.csv'))
...
def testSigSVM():
    ...
    ssvm_df.to_csv(Path('results/Sig-SVM_TestPerformance_DCTData.csv'))
...
def testKNN():
    ...
    knn_df = testKNNClassifier(testID='KNN-augmPatLvDiv',
                               train_df=augmPatLvDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':19, 'PCA_Feats':300},
                               results_df=knn_df)
    knn_df = testKNNClassifier(testID='KNN-augmRndDiv',
                               train_df=augmRndDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':15, 'PCA_Feats':1024},
                               results_df=knn_df)
    knn_df = testKNNClassifier(testID='KNN-patLvDiv',
                               train_df=patLvDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':11, 'PCA_Feats':150},
                               results_df=knn_df)
    knn_df = testKNNClassifier(testID='KNN-rndDiv',
                               train_df=rndDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':15, 'PCA_Feats':400},
                               results_df=knn_df)
    cols = knn_df.columns.tolist()
    cols.remove('testID')
    knn_df = knn_df[['testID'] + cols]
    knn_df.to_csv(Path('results/KNN_TestPerformance_DCTData.csv'))
...
def testRF():
    ...
    rf_df = testRFClassifier(testID='RF-augmPatLvDiv',
                              train_df=augmPatLvDiv_TRAIN,
                              funcPointer=RF_CLF,
                              funcParams={'n_estimators':70, 'PCA_Feats':50},
                              results_df=rf_df)
    rf_df = testRFClassifier(testID='RF-augmRndDiv',
                              train_df=augmRndDiv_TRAIN,
                              funcPointer=RF_CLF,
                              funcParams={'n_estimators':130, 'PCA_Feats':50},
                              results_df=rf_df)
    rf_df = testRFClassifier(testID='RF-patLvDiv',
                              train_df=patLvDiv_TRAIN,
                              funcPointer=RF_CLF,
                              funcParams={'n_estimators':130, 'PCA_Feats':100},
                              results_df=rf_df)
    ...
    rf_df.to_csv(Path('results/RF_TestPerformance_DCTData.csv'))
...
def testNN():
    printTable(Path('results/NN_TestPerformance_DCTData.csv'))
...
if __name__ == '__main__':
    ...
    #testLSVM()
    #printTable(Path('results/L-SVM_TestPerformance_DCTData.csv'))

    #testQSVM()
    #printTable(Path('results/Q-SVM_TestPerformance_DCTData.csv'))
    
    #testPSVM()
    #printTable(Path('results/P-SVM_TestPerformance_DCTData.csv'))

    #testRSVM()
    #printTable(Path('results/R-SVM_TestPerformance_DCTData.csv'))
    
    #testSigSVM()
    #printTable(Path('results/Sig-SVM_TestPerformance_DCTData.csv'))
    
    ###############################################################################
    
    #testKNN()
    #printTable(Path('results/KNN_TestPerformance_DCTData.csv'))
    
    ###############################################################################
    
    #testRF()
    #printTable(Path('results/RF_TestPerformance_DCTData.csv'))
