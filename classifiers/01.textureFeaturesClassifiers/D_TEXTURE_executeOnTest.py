...
from B_SKLearnClassifier_TexFeats import balanceDataset, computeStats, \
                                    Xy_Split, validateCLF

from B_SKLearnClassifier_TexFeats import L_SVM_CLF, Q_SVM_CLF, Poly_SVM_CLF, RBF_SVM_CLF, Sigm_SVM_CLF,\
                                         KNN_CLF, RF_CLF
...
def testLSVM():
    ...
    lsvm_df = testSVMClassifier(testID='LSVM-augmPatLvDiv',
                                ...
                                funcParams={'params':{'C':1.0, 'tol':0.001, 'max_iter':-1}, 'PCA_Feats':60},
                                results_df=lsvm_df)
    ...
    lsvm_df = testSVMClassifier(testID='LSVM-patLvDiv',
                                ...
                                funcPointer=L_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'tol':0.001, 'max_iter':-1}, 'PCA_Feats':55},
                                results_df=lsvm_df)
    lsvm_df = testSVMClassifier(testID='LSVM-rndDiv',
                                train_df=rndDiv_TRAIN,
                                funcPointer=L_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'tol':0.001, 'max_iter':-1}, 'PCA_Feats':70},
                                results_df=lsvm_df)
    cols = lsvm_df.columns.tolist()
    cols.remove('testID')
    lsvm_df = lsvm_df[['testID'] + cols]
    lsvm_df.to_csv(Path('results/L-SVM_TestPerformance_TextureData.csv'))
...
def testQSVM():
    ...
    qsvm_df = testSVMClassifier(testID='QSVM-augmPatLvDiv',
                                ...
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':25},
                                results_df=qsvm_df)
    ...
    qsvm_df.to_csv(Path('results/Q-SVM_TestPerformance_TextureData.csv'))
...
def testPSVM():
    ...
    psvm_df = testSVMClassifier(testID='PSVM-augmRndDiv',
                                train_df=augmRndDiv_TRAIN,
                                funcPointer=Poly_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':1e6}, 'PCA_Feats':40},
                                results_df=psvm_df)
    ...
    psvm_df = testSVMClassifier(testID='PSVM-rndDiv',
                                train_df=rndDiv_TRAIN,
                                funcPointer=Poly_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':1e6}, 'PCA_Feats':25},
                                results_df=psvm_df)
    cols = psvm_df.columns.tolist()
    cols.remove('testID')
    psvm_df = psvm_df[['testID'] + cols]
    psvm_df.to_csv(Path('results/P-SVM_TestPerformance_TextureData.csv'))
...
def testRSVM():
    ...
    rsvm_df = testSVMClassifier(testID='RSVM-augmRndDiv',
                                train_df=augmRndDiv_TRAIN,
                                funcPointer=RBF_SVM_CLF,
                                funcParams={'params':{'C':1.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':15},
                                results_df=rsvm_df)
    ...
    rsvm_df.to_csv(Path('results/R-SVM_TestPerformance_TextureData.csv'))
...
def testSigSVM():
    ...
    ssvm_df = testSVMClassifier(testID='SigSVM-augmRndDiv',
                                train_df=augmRndDiv_TRAIN,
                                funcPointer=Sigm_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':30},
                                results_df=ssvm_df)
    ...
    ssvm_df.to_csv(Path('results/Sig-SVM_TestPerformance_TextureData.csv'))
...
def testKNN():
    ...
    knn_df = testKNNClassifier(testID='KNN-augmPatLvDiv',
                               train_df=augmPatLvDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':7, 'PCA_Feats':35},
                               results_df=knn_df)
    knn_df = testKNNClassifier(testID='KNN-augmRndDiv',
                               train_df=augmRndDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':18, 'PCA_Feats':30},
                               results_df=knn_df)
    knn_df = testKNNClassifier(testID='KNN-patLvDiv',
                               train_df=patLvDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':19, 'PCA_Feats':15},
                               results_df=knn_df)
    knn_df = testKNNClassifier(testID='KNN-rndDiv',
                               train_df=rndDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':11, 'PCA_Feats':10},
                               results_df=knn_df)
    cols = knn_df.columns.tolist()
    cols.remove('testID')
    knn_df = knn_df[['testID'] + cols]
    knn_df.to_csv(Path('results/KNN_TestPerformance_TextureData.csv'))
...
def testRF():
    ...
    rf_df = testRFClassifier(testID='RF-augmPatLvDiv',
                              train_df=augmPatLvDiv_TRAIN,
                              funcPointer=RF_CLF,
                              funcParams={'n_estimators':70, 'PCA_Feats':None},
                              results_df=rf_df)
    rf_df = testRFClassifier(testID='RF-augmRndDiv',
                              train_df=augmRndDiv_TRAIN,
                              funcPointer=RF_CLF,
                              funcParams={'n_estimators':150, 'PCA_Feats':70},
                              results_df=rf_df)
    ...
    rf_df.to_csv(Path('results/RF_TestPerformance_TextureData.csv'))
...
def testNN():
    printTable(Path('results/NN_TestPerformance_TextureData.csv'))
...
if __name__ == '__main__':
    ...
    #testLSVM()
    #printTable(Path('results/L-SVM_TestPerformance_TextureData.csv'))

    #testQSVM()
    #printTable(Path('results/Q-SVM_TestPerformance_TextureData.csv'))
    
    #testPSVM()
    #printTable(Path('results/P-SVM_TestPerformance_TextureData.csv'))

    #testRSVM()
    #printTable(Path('results/R-SVM_TestPerformance_TextureData.csv'))
    
    #testSigSVM()
    #printTable(Path('results/Sig-SVM_TestPerformance_TextureData.csv'))
    
    ###############################################################################
    
    #testKNN()
    #printTable(Path('results/KNN_TestPerformance_TextureData.csv'))
    
    ###############################################################################
    
    #testRF()
    #printTable(Path('results/RF_TestPerformance_TextureData.csv'))
