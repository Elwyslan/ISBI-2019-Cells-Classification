...
augmPatLvDiv_TRAIN = Path('data/AugmPatLvDiv_TRAIN-FOFData_108-Features_20000-images.csv')
augmPatLvDiv_VALIDATION = Path('data/AugmPatLvDiv_VALIDATION-FOFData_108-Features_5000-images.csv')
augmRndDiv_TRAIN = Path('data/AugmRndDiv_TRAIN-FOFData_108-Features_20000-images.csv')
augmRndDiv_VALIDATION = Path('data/AugmRndDiv_VALIDATION-FOFData_108-Features_5000-images.csv')
patLvDiv_TEST = Path('data/PatLvDiv_TEST-FOFData_108-Features_550-images.csv')
patLvDiv_TRAIN = Path('data/PatLvDiv_TRAIN-FOFData_108-Features_7224-images.csv')
patLvDiv_VALIDATION = Path('data/PatLvDiv_VALIDATION-FOFData_108-Features_2887-images.csv')
rndDiv_TEST = Path('data/rndDiv_TEST-FOFData_108-Features_1066-images.csv')
rndDiv_TRAIN = Path('data/rndDiv_TRAIN-FOFData_108-Features_6398-images.csv')
rndDiv_VALIDATION = Path('data/rndDiv_VALIDATION-FOFData_108-Features_3197-images.csv')

from B_SKLearnClassifier_FOF import balanceDataset, computeStats, \
                                    Xy_Split, validateCLF

from B_SKLearnClassifier_FOF import L_SVM_CLF, Q_SVM_CLF, Poly_SVM_CLF, RBF_SVM_CLF, Sigm_SVM_CLF,\
                                    KNN_CLF, RF_CLF
...
def testLSVM():
    ...
    lsvm_df = testSVMClassifier(testID='LSVM-augmPatLvDiv',
                                ...
                                funcParams={'params':{'C':1.0, 'tol':0.001, 'max_iter':-1}, 'PCA_Feats':85},
                                results_df=lsvm_df)
    lsvm_df = testSVMClassifier(testID='LSVM-augmRndDiv',
                                train_df=augmRndDiv_TRAIN,
                                funcPointer=L_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'tol':0.001, 'max_iter':-1}, 'PCA_Feats':100},
                                results_df=lsvm_df)
    lsvm_df = testSVMClassifier(testID='LSVM-patLvDiv',
                                train_df=patLvDiv_TRAIN,
                                funcPointer=L_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'tol':0.001, 'max_iter':-1}, 'PCA_Feats':80},
                                results_df=lsvm_df)
    ...
    lsvm_df.to_csv(Path('results/L-SVM_TestPerformance_FOFData.csv'))
...
def testQSVM():
    ...
    qsvm_df = testSVMClassifier(testID='QSVM-augmPatLvDiv',
                                ...
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':55},
                                results_df=qsvm_df)
    qsvm_df = testSVMClassifier(testID='QSVM-augmRndDiv',
                                train_df=augmRndDiv_TRAIN,
                                funcPointer=Q_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':70},
                                results_df=qsvm_df)
    qsvm_df = testSVMClassifier(testID='QSVM-patLvDiv',
                                train_df=patLvDiv_TRAIN,
                                funcPointer=Q_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':40},
                                results_df=qsvm_df)
    qsvm_df = testSVMClassifier(testID='QSVM-rndDiv',
                                train_df=rndDiv_TRAIN,
                                funcPointer=Q_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':50},
                                results_df=qsvm_df)
    cols = qsvm_df.columns.tolist()
    cols.remove('testID')
    qsvm_df = qsvm_df[['testID'] + cols]
    qsvm_df.to_csv(Path('results/Q-SVM_TestPerformance_FOFData.csv'))
...
def testPSVM():
    ...
    psvm_df = testSVMClassifier(testID='PSVM-augmPatLvDiv',
                                ...
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':1e6}, 'PCA_Feats':55},
                                results_df=psvm_df)
    ...
    psvm_df = testSVMClassifier(testID='PSVM-patLvDiv',
                                train_df=patLvDiv_TRAIN,
                                funcPointer=Poly_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':1e6}, 'PCA_Feats':35},
                                results_df=psvm_df)
    psvm_df = testSVMClassifier(testID='PSVM-rndDiv',
                                train_df=rndDiv_TRAIN,
                                funcPointer=Poly_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':1e6}, 'PCA_Feats':30},
                                results_df=psvm_df)
    cols = psvm_df.columns.tolist()
    cols.remove('testID')
    psvm_df = psvm_df[['testID'] + cols]
    psvm_df.to_csv(Path('results/P-SVM_TestPerformance_FOFData.csv'))
...
def testRSVM():
    ...
    rsvm_df = testSVMClassifier(testID='RSVM-augmPatLvDiv',
                                ...
                                funcParams={'params':{'C':1.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':75},
                                results_df=rsvm_df)
    rsvm_df = testSVMClassifier(testID='RSVM-augmRndDiv',
                                train_df=augmRndDiv_TRAIN,
                                funcPointer=RBF_SVM_CLF,
                                funcParams={'params':{'C':1.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':45},
                                results_df=rsvm_df)
    rsvm_df = testSVMClassifier(testID='RSVM-patLvDiv',
                                train_df=patLvDiv_TRAIN,
                                funcPointer=RBF_SVM_CLF,
                                funcParams={'params':{'C':1.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':35},
                                results_df=rsvm_df)
    rsvm_df = testSVMClassifier(testID='RSVM-rndDiv',
                                train_df=rndDiv_TRAIN,
                                funcPointer=RBF_SVM_CLF,
                                funcParams={'params':{'C':1.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':40},
                                results_df=rsvm_df)
    cols = rsvm_df.columns.tolist()
    cols.remove('testID')
    rsvm_df = rsvm_df[['testID'] + cols]
    rsvm_df.to_csv(Path('results/R-SVM_TestPerformance_FOFData.csv'))
...
def testSigSVM():
    ...
    ssvm_df = testSVMClassifier(testID='SigSVM-augmPatLvDiv',
                                ...
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':None},
                                results_df=ssvm_df)
    ssvm_df = testSVMClassifier(testID='SigSVM-augmRndDiv',
                                train_df=augmRndDiv_TRAIN,
                                funcPointer=Sigm_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':95},
                                results_df=ssvm_df)
    ...
    ssvm_df = testSVMClassifier(testID='SigSVM-rndDiv',
                                train_df=rndDiv_TRAIN,
                                funcPointer=Sigm_SVM_CLF,
                                funcParams={'params':{'C':1.0, 'coef0':0.0,'tol':0.001, 'max_iter':-1}, 'PCA_Feats':100},
                                results_df=ssvm_df)
    cols = ssvm_df.columns.tolist()
    cols.remove('testID')
    ssvm_df = ssvm_df[['testID'] + cols]
    ssvm_df.to_csv(Path('results/Sig-SVM_TestPerformance_FOFData.csv'))
...
def testKNN():
    ...
    knn_df = testKNNClassifier(testID='KNN-augmPatLvDiv',
                               train_df=augmPatLvDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':7, 'PCA_Feats':20},
                               results_df=knn_df)
    knn_df = testKNNClassifier(testID='KNN-augmRndDiv',
                               train_df=augmRndDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':6, 'PCA_Feats':85},
                               results_df=knn_df)
    knn_df = testKNNClassifier(testID='KNN-patLvDiv',
                               train_df=patLvDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':13, 'PCA_Feats':25},
                               results_df=knn_df)
    knn_df = testKNNClassifier(testID='KNN-rndDiv',
                               train_df=rndDiv_TRAIN,
                               funcPointer=KNN_CLF,
                               funcParams={'n_neighbors':7, 'PCA_Feats':70},
                               results_df=knn_df)
    cols = knn_df.columns.tolist()
    cols.remove('testID')
    knn_df = knn_df[['testID'] + cols]
    knn_df.to_csv(Path('results/KNN_TestPerformance_FOFData.csv'))
...
def testRF():
    ...
    rf_df = testRFClassifier(testID='RF-augmPatLvDiv',
                              train_df=augmPatLvDiv_TRAIN,
                              funcPointer=RF_CLF,
                              funcParams={'n_estimators':150, 'PCA_Feats':75},
                              results_df=rf_df)
    rf_df = testRFClassifier(testID='RF-augmRndDiv',
                              train_df=augmRndDiv_TRAIN,
                              funcPointer=RF_CLF,
                              funcParams={'n_estimators':150, 'PCA_Feats':100},
                              results_df=rf_df)
    rf_df = testRFClassifier(testID='RF-patLvDiv',
                              train_df=patLvDiv_TRAIN,
                              funcPointer=RF_CLF,
                              funcParams={'n_estimators':110, 'PCA_Feats':25},
                              results_df=rf_df)
    rf_df = testRFClassifier(testID='RF-rndDiv',
                              train_df=rndDiv_TRAIN,
                              funcPointer=RF_CLF,
                              funcParams={'n_estimators':70, 'PCA_Feats':25},
                              results_df=rf_df)
    cols = rf_df.columns.tolist()
    cols.remove('testID')
    rf_df = rf_df[['testID'] + cols]
    rf_df.to_csv(Path('results/RF_TestPerformance_FOFData.csv'))
...
def testNN():
    printTable(Path('results/NN_TestPerformance_FOFData.csv'))
...
if __name__ == '__main__':
    ...
    #testLSVM()
    #printTable(Path('results/L-SVM_TestPerformance_FOFData.csv'))

    #testQSVM()
    #printTable(Path('results/Q-SVM_TestPerformance_FOFData.csv'))
    
    #testPSVM()
    #printTable(Path('results/P-SVM_TestPerformance_FOFData.csv'))

    #testRSVM()
    #printTable(Path('results/R-SVM_TestPerformance_FOFData.csv'))
    
    #testSigSVM()
    #printTable(Path('results/Sig-SVM_TestPerformance_FOFData.csv'))
    
    ###############################################################################
    
    #testKNN()
    #printTable(Path('results/KNN_TestPerformance_FOFData.csv'))
    
    ###############################################################################
    
    #testRF()
    #printTable(Path('results/RF_TestPerformance_FOFData.csv'))
