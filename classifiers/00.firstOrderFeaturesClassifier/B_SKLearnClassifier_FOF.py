...
if __name__ == '__main__':
    ...
    pcaNofFeat = [i for i in range(5,105,5)] + [108]
    ###############################################################################
    def LSVMaugmPatLvDiv():
        ...
        df.to_csv(f'results/L-SVM_ValidPerformance_FOFData_AugmPatLvDivImages.csv')
    ###############################################################################
    def LSVMaugmRndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'L-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/L-SVM_ValidPerformance_FOFData_AugmRndDivImages.csv')
    ###############################################################################
    def LSVMpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'L-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/L-SVM_ValidPerformance_FOFData_PatLvDivImages.csv')
    ###############################################################################
    def LSVMrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'L-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/L-SVM_ValidPerformance_FOFData_RndDivImages.csv')
    ###############################################################################
    ...
    def QSVMaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Q-SVM_ValidPerformance_FOFData_AugmPatLvDivImages.csv')
    ###############################################################################
    def QSVMaugmRndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'Q-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Q-SVM_ValidPerformance_FOFData_AugmRndDivImages.csv')
    ###############################################################################
    def QSVMpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'Q-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Q-SVM_ValidPerformance_FOFData_PatLvDivImages.csv')
    ###############################################################################
    def QSVMrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'Q-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Q-SVM_ValidPerformance_FOFData_RndDivImages.csv')
    ###############################################################################
    ...
    def PolySVMaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/P-SVM_ValidPerformance_FOFData_AugmPatLvDivImages.csv')
    ###############################################################################
    def PolySVMaugmRndDiv():
        ...
        for feat in pcaNofFeat:
            ...
            collectedData[f'P-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/P-SVM_ValidPerformance_FOFData_AugmRndDivImages.csv')
    ###############################################################################
    def PolySVMpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'P-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/P-SVM_ValidPerformance_FOFData_PatLvDivImages.csv')
    ###############################################################################
    def PolySVMrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'P-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/P-SVM_ValidPerformance_FOFData_RndDivImages.csv')
    ###############################################################################
    ...
    def RBFSVMaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/R-SVM_ValidPerformance_FOFData_AugmPatLvDivImages.csv')
    ###############################################################################
    def RBFSVMaugmRndDiv():
        ...
        for feat in pcaNofFeat:
            ...
            collectedData[f'R-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/R-SVM_ValidPerformance_FOFData_AugmRndDivImages.csv')
    ###############################################################################
    def RBFSVMpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'R-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/R-SVM_ValidPerformance_FOFData_PatLvDivImages.csv')
    ###############################################################################
    def RBFSVMrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            collectedData[f'R-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/R-SVM_ValidPerformance_FOFData_RndDivImages.csv')
    ###############################################################################
    ...
    def SigmoidSVMaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Sig-SVM_ValidPerformance_FOFData_AugmPatLvDivImages.csv')
    ###############################################################################
    def SigmoidSVMaugmRndDiv():
        ...
        for feat in pcaNofFeat:
            ...
            collectedData[f'S-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Sig-SVM_ValidPerformance_FOFData_AugmRndDivImages.csv')
    ###############################################################################
    def SigmoidSVMpatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            collectedData[f'S-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Sig-SVM_ValidPerformance_FOFData_PatLvDivImages.csv')
    ###############################################################################
    def SigmoidSVMrndDiv():
        ...
        for feat in pcaNofFeat:
            ...
            collectedData[f'S-SVM_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/Sig-SVM_ValidPerformance_FOFData_RndDivImages.csv')
    ###############################################################################
    ...
    def KNNaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/KNN_ValidPerformance_FOFData_AugmPatLvDivImages.csv')
    ###############################################################################
    def KNNaugmRndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for k in range(5,21):
                ...
                collectedData[f'{k}-KNN_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/KNN_ValidPerformance_FOFData_AugmRndDivImages.csv')
    ###############################################################################
    def KNNpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for k in range(5,21):
                ...
                collectedData[f'{k}-KNN_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/KNN_ValidPerformance_FOFData_PatLvDivImages.csv')
    ###############################################################################
    def KNNrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for k in range(5,21):
                ...
                collectedData[f'{k}-KNN_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/KNN_ValidPerformance_FOFData_rndDivImages.csv')
    ###############################################################################
    ...
    def RFaugmPatLvDiv():
        ...
        for feat in pcaNofFeat:
            ...
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/RF_ValidPerformance_FOFData_AugmPatLvDivImages.csv')
    ###############################################################################
    def RFaugmRndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for nEstm in range(50,170,20):
                ...
                collectedData[f'{nEstm}-RF_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/RF_ValidPerformance_FOFData_AugmRndDivImages.csv')
    ###############################################################################
    def RFpatLvDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for nEstm in range(50,170,20):
                ...
                collectedData[f'{nEstm}-RF_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/RF_ValidPerformance_FOFData_PatLvDivImages.csv')
    ###############################################################################
    def RFrndDiv():
        df = pd.DataFrame()
        for feat in pcaNofFeat:
            ...
            for nEstm in range(50,170,20):
                ...
                collectedData[f'{nEstm}-RF_prec'] = prec
            df = df.append(collectedData, ignore_index=True)
        df.to_csv(f'results/RF_ValidPerformance_FOFData_RndDivImages.csv')
    ###############################################################################
