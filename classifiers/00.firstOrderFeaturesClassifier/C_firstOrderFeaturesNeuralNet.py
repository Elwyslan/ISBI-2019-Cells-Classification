...
if __name__ == '__main__':
    ...
    def validPerfAugmPatLvDivImages(train_df, valid_df, test=False):
        x_train, y_train, x_valid, y_valid = prepareData(train_df, valid_df)
        model = Sequential()
        model.add(Dense(1024,activation='tanh',input_shape=(x_train.shape[1],)))
        model.add(Dropout(0.5))
        ...
        model.compile(loss='categorical_crossentropy',
                      ...
                      metrics=['accuracy'])
        print(model.summary())
        LOG_DIR = f'results/NeuralNetResults/NN_augmPatLvDiv_test/'
        if not os.path.exists(Path(LOG_DIR)):
            os.mkdir(Path(LOG_DIR))
        else:
            shutil.rmtree(Path(LOG_DIR))
    ...
    def validPerfAugmRndDivImages(train_df, valid_df, test=False):
        ...
        model.compile(loss='categorical_crossentropy',
                      ...
                      metrics=['accuracy'])
        print(model.summary())
        LOG_DIR = f'results/NeuralNetResults/NN_augmRndDiv_test/'
        if not os.path.exists(Path(LOG_DIR)):
            os.mkdir(Path(LOG_DIR))
        else:
            shutil.rmtree(Path(LOG_DIR))
    ...
    def validPerfPatLvDivImages(train_df, valid_df, test=False):
        ...
        model.compile(loss='categorical_crossentropy',
                      ...
                      metrics=['accuracy'])
        print(model.summary())
        LOG_DIR = f'results/NeuralNetResults/NN_patLvDiv_test/'
        if not os.path.exists(Path(LOG_DIR)):
            os.mkdir(Path(LOG_DIR))
        else:
            shutil.rmtree(Path(LOG_DIR))
    ...
    def validPerfRndDivImages(train_df, valid_df, test=False):
        ...
        model.compile(loss='categorical_crossentropy',
                      ...
                      metrics=['accuracy'])
        print(model.summary())
        LOG_DIR = f'results/NeuralNetResults/NN_rndDiv_test/'
        if not os.path.exists(Path(LOG_DIR)):
            os.mkdir(Path(LOG_DIR))
        else:
            shutil.rmtree(Path(LOG_DIR))
    ...
    print(nn_df.head())
    nn_df.to_csv(Path('results/NN_TestPerformance_FOFData.csv'))
